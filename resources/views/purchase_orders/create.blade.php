...
$(document).ready(function() {
    let poItemIndex = {{ old('items') ? count(old('items')) : 0 }};
    ...
    function addProductToPoTable(product) {
        ...
        if (existingRow) {
            ...
            updateRowSubtotal($('#poItemsTableBody .po-item-row').last());
            poItemIndex++;
        }
        updateGrandTotalPo();
    }
    ...
    @if ($preselectedProduct)
        addProductToPoTable({
            ...
            purchase_price: {{ $preselectedProduct->purchase_price ?? 0 }}
        });
    @endif
    ...
    $('#productSearchPo').on('input', function() {
        ...
        if (searchTerm.length >= 1) {
            ...
            searchPoTimeout = setTimeout(function() {
                $.ajax({
                    url: "{{ route('products.searchForPo') }}",
                    type: 'GET',
